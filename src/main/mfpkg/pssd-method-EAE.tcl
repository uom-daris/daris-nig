...
# MRI scan
# ================================
proc createMRMethod { { action 0 } { fillin } } {
...
proc createFixMethod { { action 0 } { fillin 0 } } {
	...
	set args "${margs} \
	    ...
	    :description ${description} \
            ...
            :step < \
                ...
                :subject -part p <\
                   :metadata < \
                      :definition -requirement mandatory nig-daris:pssd-EAE-optic-nerve-removal  \
                      :value < \
                        :method ${value2a} \
                      > \
                   >\
...
proc createOMMethod { { action 0 } { fillin 0 } } {	
    ...
    set args "${margs} \
       ...
       :step < \
            ...
            :subject -part p <\
               :metadata < \
                   :definition -requirement mandatory nig-daris:pssd-EAE-optic-nerve-section  \
                   :value <\
        ...
        :step < \
            ...
            :subject -part p <\
               :metadata < \
                   :definition -requirement mandatory nig-daris:pssd-EAE-stain  \
                   :value < \
                     :agent < \
                       :name ${value2a} \
                     >\
        ...
        :step < \
            ...
            :study < \
               :type ${type} \
               :metadata <\
                 :definition -requirement mandatory nig-daris:pssd-EAE-microscopy  \
                 :value <\
...
proc createEMMethod { { action 0 } { fillin 0 } } {
...
proc createOverallMethod { mrMethodId fixMethodId OMMethodId EMMethodId { action 0 } { fillin 0 } } {
...
#####################################################################################
proc create_EAE_Method { { action 0 } { fillin 0 } } {
    set mrMethodId [createMRMethod $action $fillin]
    set fixMethodId [createFixMethod $action $fillin]
    set OMMethodId [createOMMethod $action $fillin]
    set EMMethodId [createEMMethod $action $fillin]
    #
    set overallMethodId [createOverallMethod $mrMethodId $fixMethodId $OMMethodId $EMMethodId $action $fillin]
