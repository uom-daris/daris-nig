...
proc createMRMethod { { action 0 } { fillin 0 } } {
...
#
	...
	:step < \
		...
		:study < \
			...
			:metadata < \
				:definition -requirement mandatory nig-daris:pssd-time-point \
...
# If creating Method, fillin - 0 (don't fill in cid allocator space), 1 (fill in cid allocator space)
#
proc createCRMethod { { action 0 } {fillin 0 } } {
...
#
	...
	:step < \
		...
		:study < \
			...
			> \
			:metadata < \
				:definition -requirement mandatory nig-daris:pssd-ImageHD-combined \
...
# If creating Method, fillin - 0 (don't fill in cid allocator space), 1 (fill in cid allocator space)
#
proc createMethodForSubject_controls { mrMethodId { action 0 } { fillin } } {
	...
	set args "${margs} \
		...
		:subject < \
        		:project < \
        			:public < \
        				:metadata < \
        					:definition -requirement optional nig-daris:pssd-subject  \
        					:value < :control constant(true) :type constant(animal) > \
        				> \
        				:metadata < \
        					:definition -requirement optional nig-daris:pssd-animal-subject \
        				...
        				:metadata < :definition -requirement optional nig-daris:pssd-subject-exclusion >\
        			> \
        			:private < \
...
# If creating Method, fillin - 0 (don't fill in cid allocator space), 1 (fill in cid allocator space)
#
proc createMethodForSubject_patients { mrMethodId crMethodId { action 0 } { fillin 0 } } {
...
# Create all Methods                                                         #
#============================================================================#
proc create_ImageHD_method { { action 0 } { fillin 0 } } {
    set mrMethodId [createMRMethod $action $fillin]
    set crMethodId [createCRMethod $action $fillin]
#
    set sbjControlsMethodId [createMethodForSubject_controls $mrMethodId $action $fillin]
    set sbjPatientsMethodId [createMethodForSubject_patients $mrMethodId $crMethodId $action $fillin]
}
