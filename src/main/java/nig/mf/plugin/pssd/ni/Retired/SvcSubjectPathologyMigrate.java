...
/**
 * Specialized function to migrate the Pathology meta-data element from Subject:nig-daris:pssd-animal-subject/pathology to
 * Subject:nig-daris:pssd-animal-disease/disease Once the migration is complete, the animal-mri-simple Method will only create
...
public class SvcSubjectPathologyMigrate extends PluginService {
	...
	public String description() {
		return "Specialized service to migrate Subject:hfi.pssd/human.subject/pathology to Subject:nig-daris:pssd-animal-disease";
	}
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w) throws Throwable {
		...
		for (String id : subjects) {
			...
			if (PSSDUtil.isReplica(executor(), cid)) {
				...
				String pathStr = r2.value("asset/meta/nig-daris:pssd-animal-subject/pathology");

				// If pathology exists, transfer to all Subjects associated with this R-Subject
				if (pathStr != null) {
					w.add("Subject", AssetUtil.getCid(executor(), id));

					// Transfer the Pathology information from nig-daris:pssd-animal-subject/pathology to
					// Subject/nig-daris:pssd-animal-disease
					doc = new XmlDocMaker("args");
					doc.add("id", id);
					doc.push("meta");
					doc.push("nig-daris:pssd-animal-disease", new String[] { "ns", "pssd.public" });
					...
					// Remove Pathology element from nig-daris:pssd-animal-subject meta
					XmlDoc.Element asMeta = r2.element("asset/meta/nig-daris:pssd-animal-subject");
					XmlDoc.Element pMeta = r2.element("asset/meta/nig-daris:pssd-animal-subject/pathology");
					asMeta.remove(pMeta);
