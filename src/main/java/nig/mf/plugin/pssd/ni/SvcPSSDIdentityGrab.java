...
public class SvcPSSDIdentityGrab extends PluginService {
	...
	public String description() {
		...
		return "This service extracts the Patient ID out of the DICOM meta-data and insert into the nig-daris:pssd-identity meta-data on the PSSD Subject";
	...
	private void rebuildSubjectIdentities(String subjectCid, XmlWriter w) throws Throwable {
		...
		Collection<XmlDoc.Element> PSSDIdentities = r1.elements("asset/meta/nig-daris:pssd-identity[@ns='pssd.public']");
		...
		for (String[] pIdInfo : patientIds) {
			...
			if (station.equals("EPT")) {
				newIdType = "Forey Small Animal MR Facility";
			} else if (station.equals("MRC35113")) {
				newIdType = "Royal Children's Hospital";
			} else if (station.equals("CTAWP71222")) {
				// This is the PET/CT
				// The MR will have a new station
				// They both are from the same authority
				newIdType = "Melbourne Brain Centre Imaging Unit";
			} else {
				newIdType = "Other";
			}
			...
			if(!identityExists(PSSDIdentities,newIdType,newIdValue)){
				...
				doc.push("nig-daris:pssd-identity",new String[]{"ns","pssd.public"});
				...
				w.push("nig-daris:pssd-identity",new String[]{"ns","pssd.public"});
