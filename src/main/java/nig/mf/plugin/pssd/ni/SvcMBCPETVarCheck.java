...
public class SvcMBCPETVarCheck extends PluginService {
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out,
        ...
        if (dicomPatient == null) {
            String error = "nig.pssd.mbc.petvar.check : Could not locate mf-dicom-patient record on Subject "
                    + subject;
        ...
        if (status[0] == -1 && status[1] == -1) {
            ...
            if (status[0] == 0) {
                ...
                w.add("FMP-visit-index", new String[] {
                        "Study-CID-does-not-pre-exist", studyCID }, status[1]);
            } else if (status[0] == 1) {
                // We did not find a Visit with a Subject CID only, but we did
                // find a visit for which this Study CID was already set
                // That should just mean we are redoing the same one already
                // done
                w.add("FMP-visit-index",
                        new String[] { "Study-CID-pre-exists", studyCID },
                        status[1]);
                mbc.closeConnection();
                return;
            }
     ...
     * 
     * @param rs
     * @return status [0] 1 if a visit with the Study CID is already set [0] 0
     *         if a visit is found where the CID has depth less than a STudy
     *         (usually a SUbject) [0] -1 something is wrong [1] The visit index
     *         found [1] -1 something is wrong
     * @throws Throwable
     */
