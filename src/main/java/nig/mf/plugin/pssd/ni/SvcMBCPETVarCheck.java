...
public class SvcMBCPETVarCheck extends PluginService {
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out, XmlWriter w)
        ...
        XmlDoc.Element petDICOMMeta = null;
        if (petDataSetCID != null) {
            petDICOMMeta = dicomMetaData(executor(), petDataSetCID);
        }
        XmlDoc.Element ctDICOMMeta = null;
        if (ctDataSetCID != null) {
            ctDICOMMeta = dicomMetaData(executor(), ctDataSetCID);
        }
        // Find one "acquisition date" from the DataSets. We need to fetch this
        // from the DICOM header as its not in indexed meta-data.
        Date date = findDate(executor(), petDICOMMeta, ctDICOMMeta, studyCID,
                email);
        ...
        XmlDoc.Element subjectMeta = AssetUtil.getAsset(executor(), subject,
                null);
        ...
        if (dicomPatient == null) {
            System.out
                    .println("nig.pssd.mbc.petvar.check : Could not locate mf-dicom-patient record on Subject "
                            + subject);
            throw new Exception(
                    "Could not locate mf-dicom-patient record on Subject "
                            + subject);
        }
        ...
        // There may be one (usual) or two (the 'blood sample' which always
        // comes first)
        ...
        try {
            ...
            String error = "nig.pssd.mbc.petvar.check : For DataSet '"
                    + petDataSetCID
                    + "' no PET visit found in FMP for subject " + patientName
                    + "(MBCID=" + mbcPatientID + ") and date "
                    + date.toString() + "\n";
        ...
        if (n != 1 && n != 2) {
            String error = "For DataSet '" + petDataSetCID + "' found " + n
                    + " PET visits (expecting 1 or 2) in FMP for patient "
        ...
        String diff = compareSetValues(executor(), studyCID, mbc, date,
                petDICOMMeta, ctDICOMMeta, petVisits, patientName,
                mbcPatientID, petDataSetCID, ctDataSetCID);
    ...
    private String findPETDataSet(ServiceExecutor executor, String cid)
            throws Throwable {
        XmlDocMaker dm = new XmlDocMaker("args");
        dm.add("pdist", "0");
    ...
    private void send(ServiceExecutor executor, String email, String body)
            throws Throwable {
     ...
     * 
     * @param mbc
     *            The MBC FMP data base
     * @param date
     *            The date of the acquisition of interest
     * @param petDICOMMeta
     * @param ctDICOMMeta
     * @param petVisits
     * @param patientName
     * @param mbcPatientID
     * @param dataset
     *            The citable ID of the DaRIS DataSet
     * @return
     * @throws Throwable
     */
    private String compareSetValues(ServiceExecutor executor, String studyCID,
            MBCFMP mbc, Date date, XmlDoc.Element petDICOMMeta,
            XmlDoc.Element ctDICOMMeta, ResultSet petVisits,
            String patientName, String mbcPatientID, String petCID, String ctCID)
            throws Throwable {
        ...
        if (petCID != null) {
            ...
            if (rp == null) {
                throw new Exception(
                        "No Radiopharmaceutical Information Sequence(0054,0016) found in PET series: "
                                + petCID);
            }
            ...
            // System.out.println("DICOM dose = " + doseDICOM);
            //
            startInjectionTimeDICOM = rp2.value("de[@tag='00181072']/value");
            // Rob enters just hh:mm in FMP
            startInjectionTimeDICOM2 = startInjectionTimeDICOM.substring(0, 4);
            // System.out.println("DICOM injection time = " +
            // startInjectionTimeDICOM2);
            // Acquisition Time
            petScanTimeDICOM = petDICOMMeta.value("de[@tag='00080032']/value");
            // System.out.println("PET DICOM Scan time = " + petScanTimeDICOM);
        }
        ...
        if (ctCID != null) {
            ...
            // System.out.println("CT DICOM Scan time = " + ctScanTimeDICOM);
        }
        ...
        // Is this the special 'blood pool' PET DataSet. It is guarenteed that
        // the string '1_9' will
        ...
        while (petVisits.next()) {
            ...
            // Fetch FMP values
            System.out.println("PET VIsit " + iVisit++);
             ...
             * System.out.println("   FMP injection time '" +
             * startInjectionTimeFMP + "'");
             */

            // Compare the right DICOM values with the right PET Visit (first
            // visit [blood pool]
            // or second visit [normal]). Each Study we handle is either for the
            // blood pool visit or not
            if ((scanTypeFMP.contains("EARLY") && isBloodPool)
                ...
                if (petCID != null) {
                    ...
                    if (!doseFMP.isEmpty()) {
                        ...
                        String s2 = String.format("%.1f%n", f2).trim();
                        // System.out.println("Dose : " + s1 + " " + s2);
                        // s2 = "380.0";
        ...
        if (petCID != null) {
            if (!matchedInjectionTime) {
                ...
                error += "\nFor " + patientName + "(MBC ID=" + mbcPatientID
                        ...
                        + DateUtil.formatDate(date, "dd-MMM-yyyy")
                        + " : DICOM ( " + t2 + " ) and FMP " + t
                        + " injection start times do not agree "
                        + " (DataSet='" + petCID + "')";
                err = true;
            }
            ...
            if (!matchedDose) {
                ...
                error += "\nFor " + patientName + "(MBC ID=" + mbcPatientID
                        ...
                        + " dose values do not agree " + " (DataSet='" + petCID
                        + "')";
                err = true;
            }
        }
