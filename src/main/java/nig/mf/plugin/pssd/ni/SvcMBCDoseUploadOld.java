...
public class SvcMBCDoseUploadOld extends PluginService {
    ...
    public SvcMBCDoseUploadOld() throws Throwable {
    ...
    @Override
    public String name() {
        return "nig.pssd.mbic.old.dose.upload";
    }
    ...
    public void execute(XmlDoc.Element args, Inputs in, Outputs out,
        ...
        try {
            ...
            w.add("error", msg);
            if (email != null) {
                SvcMBCPETVarCheckOld.send(executor(), email, msg);
            }
    ...
    private void updateFMP(ServiceExecutor executor, int findMethod, String studyCID, MBCFMP mbc,
        ...
        // Number of visits
        int nVisits = SvcMBCPETVarCheckOld.numberVisits(petVisits);
        ...
        while (petVisits.next()) {
            if (iVisit == visitIdx) {
                ...
                if (update) {
                    mbc.updatePETCTDoseLegacy(mbcPatientID, date, null, doseLengthProductTotal,
