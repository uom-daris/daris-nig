...
public class SvcMBCDoseUpload extends PluginService {
	...
	public void execute(XmlDoc.Element args, Inputs in, Outputs out,
		...
		if (petVisit==null) {
			...
			if (SQLUtil.sizeResultSet(petVisit) != 1) {
				String error = "nig.pssd.mbic.dose.upload : There must be precisely one Visit in " +
		...
		// Do update
		try {
			update(executor(), mbc, updateFMP, studyCID, fmpVisitID, petVisit, findMethod, iMax,  w);
	...
	private void update(ServiceExecutor executor,  MBCFMP mbc, Boolean updateFMP,
			String studyCID, String fmpVisitID, ResultSet petVisit, int findMethod, Integer iMax, XmlWriter w) throws Throwable {
		...
		try {
			updateFMP(executor, findMethod, studyCID, fmpVisitID, petVisit, mbc, dicomMeta, doseReport,
					updateFMP, w);
		} catch (Throwable t) {
			mbc.closeConnection();
	...
	private void updateFMP(ServiceExecutor executor, int findMethod, String studyCID, String fmpVisitID, ResultSet petVisit, MBCFMP mbc,
		...
		// Get values from FMP Visit
		petVisit.beforeFirst();
		petVisit.next();
		String fmpCTDoseLengthProductTotal = petVisit.getString("DLPmGycmCT2");
		String fmpXRayModulationType = petVisit.getString("XRayModulationType");
		String fmpDLP = petVisit.getString("DLPmGycmCT");
		String fmpkVp = petVisit.getString("kVp");
		String fmpXRayCurrent = petVisit.getString("ReferencemAs");
		w.push("FMP");
		...
		if (update) {
			mbc.updatePETCTDose(fmpVisitID, doseLengthProductTotal,
