...
public class SvcMBCDoseUpload extends PluginService {
	...
	public SvcMBCDoseUpload() throws Throwable {
		...
		_defn.add(me);
		//
		me = new Interface.Element("id", AssetType.DEFAULT, "The asset ID (not citeable) of the parent Study holding the SR DICOM modality DataSet..",
				0, 1);
		_defn.add(me);
		//
	...
	private void update (ServiceExecutor executor, Boolean updateFMP, Boolean dbg, String studyCID, XmlWriter w) throws Throwable {
		...
		String modality = seriesMeta.value("asset/meta/mf-dicom-series/modality");
		if (modality==null) {
			throw new Exception ("No DICOM modality found for DataSet " + srCID);
		}
		if (!modality.equals("SR")) {
			throw new Exception ("Unexpected modality (" + modality + ") for DataSet " +srCID);
		}

		// Extract the Dose report
		XmlDocMaker dm = new XmlDocMaker("args");
		dm.add("cid", srCID);
		XmlDoc.Element doseReport = executor().execute("daris.dicom.sr.get", dm.root());
		if (doseReport==null) {
			throw new Exception ("Null dose report for DICOM SR DataSet " + srCID);
		}
		...
		if (dicomMeta==null) {
			throw new Exception ("Failed to extract DICOM meta-data from SR dataSet " + srCID);
		}
	...
	private String findSR (ServiceExecutor executor, String studyCID) throws Throwable {
		XmlDocMaker dm = new XmlDocMaker("args");
		String where = "cid starts with '" + studyCID + "' and xpath(mf-dicom-series/modality)='SR'";
		dm.add("pdist", "0");
		dm.add("where", where);
		...
		if (ids.size()>1) {
			throw new Exception ("Found " + ids.size() + " SR modality DataSets under Study " + studyCID + " - cannot handle");
		}
	...
	private void updateFMP (MBCFMP mbc, XmlDoc.Element dicomMeta, XmlDoc.Element doseReport, Boolean update, Boolean dbg, XmlWriter w) throws Throwable {
		...
		if (sdate==null) {
			throw new Exception ("The study date is null in the  DICOM meta-data");
		}
		w.add("date", sdate);
		Date date = DateUtil.dateFromString(sdate, "dd-MMM-yyyy");


		// Fetch the required values from the dose report and specifically desired CT acquisition
		String t  = 
				doseReport.value("dicom-structured-report/item/item[@name='CT Accumulated Dose Data']/item[@name='CT Dose Length Product Total']");
		if (t==null) {
			throw new Exception ("Failed to extract doseLengthProductTotal from dose report.");
		}
		...
		if (acq==null) {
			throw new Exception ("Failed to find the correct CT Acquisition in the dose report");
		}


		// XRay Modulation Type
		//   :item -name "Comment" -relationship "CONTAINS" -code "121106" -type "TEXT" "Internal technical scan parameters: Organ Characteristic = Head, Body Size = Adult, Body Region = Head, X-ray Modulation Type = Z_EC"
		...
		if (XRayModType==null) {
			throw new Exception ("Failed to extract XRayModType from CT acquisition");
		}
		w.add("XRayModulationType", XRayModType);

		// DLP
		// :item -name "DLP" -relationship "CONTAINS" -code "113838" -type "NUM" "269.77 mGycm"
		...
		//   :item -name "KVP" -relationship "CONTAINS" -code "113733" -type "NUM" "100 kV"
		XmlDoc.Element kVpEl = acq.element("item[@name='CT Acquisition Parameters']/item[@name='CT X-Ray Source Parameters']/item[@name='KVP']");
		...
		// XRay Tube Current
		// :item -name "X-Ray Tube Current" -relationship "CONTAINS" -code "113734" -type "NUM" "72 mA"

		XmlDoc.Element currentEl = acq.element("item[@name='CT Acquisition Parameters']/item[@name='CT X-Ray Source Parameters']/item[@name='X-Ray Tube Current']");
		String currentStr = currentEl.value();
		parts = currentStr.split(" ");
		String current =  parts[0].trim();
		if (current==null) {
			throw new Exception ("Failed to extract current from CT acquisition");
		}
	...
	private String getDicomValue (XmlDoc.Element dicomMeta, String  tag) throws Throwable {
		XmlDoc.Element el = dicomMeta.element("de[@tag='"+tag+"']");
		return el.value("value");
	}

	private XmlDoc.Element findAcquisition (XmlDoc.Element doseReport) throws Throwable {


		// dicom-structured-report
		//   :item -name "X-Ray Radiation Dose Report"
		//       :item -name "CT Acquisition" 
		//           :item -name "Acquisition Protocol" -relationship "CONTAINS" -code "125203" -type "TEXT" "Topogram"
		// FInd first all the CT Acquisition elements that we want (the ones without TOPOGRAM)	

		Collection<XmlDoc.Element> els = 
				doseReport.elements("dicom-structured-report/item[@name='X-Ray Radiation Dose Report']/item[@name='CT Acquisition']");
		if (els==null) {
			throw new Exception ("Could not extract any CT Acquisition elements from dose report");
		}
		...
		// Now we have the list of CT acquisitions. FInd the one with the largest DLP
		int n = acqs.size();
		if (n==0) {
			throw new Exception ("Could not extract any non-TOPOGRAM CT Acquisition elements from dose report");
		}
		int idx = 0;
		Double dlpMax = -1.0;
		for (int i=0; i<n; i++) {
		...
		// OK now we have the index for the acquisition with the largest value of DLP
